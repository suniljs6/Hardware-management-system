...
<?php
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql2 = "select cid from Components where ? in (select name from Components)";
            $qq = $pdo->prepare($sql2);
            $qq->execute(array($name));
            $sql3="select cid from Components where ? in (select type from Components)";
            $qq1 = $pdo->prepare($sql3);
            $qq1->execute(array($type));
            $sql4="select cid from Components where ? in (select company from Components)";
            $qq2 = $pdo->prepare($sql4);
            $qq2->execute(array($brand));
            $sql5="select cid from Components where ? in (select cost from Components)";
            $qq3 = $pdo->prepare($sql5);
            $qq3->execute(array($cost));
            
            if($qq and $qq1 and $qq2 and $qq3){
            	$sq = "update Components set quantity=quantity+? where name=? and type=? and company=? and cost=?";
            	$ret=$pdo->prepare($sq);
            	$ret->execute(array($quantity,$name,$type,$brand,$cost));
            	Database::disconnect();
